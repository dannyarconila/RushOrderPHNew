...
-- Configurable retry rider strategy
--
-- all_eligible:
--   Re-dispatch to all currently eligible riders on every retry.
--
-- exclude_timed_out:
--   Do not re-dispatch to riders whose previous offer for this
--   job already expired/timed out.
--
...
VALUES (
  ...
  to_jsonb('exclude_timed_out'::text),
  'Retry rider strategy: all_eligible | exclude_timed_out',
  false
)
...
-- Keep the function available only to the expected callers.
REVOKE ALL
