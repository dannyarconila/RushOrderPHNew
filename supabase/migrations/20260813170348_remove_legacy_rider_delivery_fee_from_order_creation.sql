-- ============================================================
-- Remove legacy rider_delivery_fee from new Marketplace orders.
--
-- Rider platform fees are charged separately at rider acceptance
-- through calculate_rider_platform_fee().
--
-- rider_commission is preserved as a historical/earnings field
-- and is no longer populated from the obsolete rider_delivery_fee.
-- ============================================================
...
BEGIN
  ...
  IF jsonb_typeof(COALESCE(_items, '[]'::jsonb)) <> 'array'
     OR jsonb_array_length(COALESCE(_items, '[]'::jsonb)) = 0 THEN
    RAISE EXCEPTION 'Your cart is empty.';
  END IF;

  advisory_lock_key :=
    hashtextextended(uid::text || ':' || btrim(_idempotency_key), 0);
  ...
  IF store_row.latitude IS NULL
     OR store_row.longitude IS NULL
     OR address_row.latitude IS NULL
     OR address_row.longitude IS NULL THEN
  ...
  IF COALESCE(store_row.delivery_radius_km, 0) > 0
     AND distance_amount > store_row.delivery_radius_km THEN
    RAISE EXCEPTION 'The store is currently unavailable.';
  END IF;
  ...
  has_dispatch_pricing :=
    dispatch_cfg ? 'dispatch_fee_per_km'
    OR dispatch_cfg ? 'dispatch_min_fee'
    OR dispatch_cfg ? 'dispatch_max_fee';

  fee_per_km := CASE
    WHEN dispatch_cfg ? 'dispatch_fee_per_km'
      THEN COALESCE(
        trim(both '"' FROM (dispatch_cfg -> 'dispatch_fee_per_km')::text)::numeric,
        0
      )
    ELSE 0
  END;
  ...
  min_fee := CASE
    WHEN dispatch_cfg ? 'dispatch_min_fee'
      THEN COALESCE(
        trim(both '"' FROM (dispatch_cfg -> 'dispatch_min_fee')::text)::numeric,
        0
      )
    ELSE 0
  END;
  ...
  max_fee := CASE
    WHEN dispatch_cfg ? 'dispatch_max_fee'
      THEN COALESCE(
        trim(both '"' FROM (dispatch_cfg -> 'dispatch_max_fee')::text)::numeric,
        999999999
      )
    ELSE 999999999
  END;
  ...
  LOOP
    ...
    IF NOT FOUND
       OR product_row.is_published IS DISTINCT FROM true
       OR product_row.is_available IS DISTINCT FROM true THEN
      RAISE EXCEPTION 'One or more products are no longer available.';
    END IF;
    ...
    line_subtotal_value :=
      round(COALESCE(product_row.price, 0) * requested.quantity, 2);
    ...
    SET stock = stock - requested.quantity::int,
        is_available = CASE
          WHEN stock - requested.quantity::int > 0
            THEN is_available
          ELSE false
        END,
        updated_at = now()
  ...
  IF minimum_order_amount > 0
     AND subtotal_amount < minimum_order_amount THEN
    RAISE EXCEPTION 'The store is currently unavailable.';
  END IF;

  IF store_row.delivery_fee_override IS NOT NULL
     AND store_row.delivery_fee_override >= 0 THEN
    delivery_fee_amount :=
      round(store_row.delivery_fee_override, 2);
    ...
    delivery_fee_amount := round(
      GREATEST(
        ...
        LEAST(
          max_fee,
          round(
            distance_amount * GREATEST(fee_per_km, 0),
            2
          )
        )
      ),
  ...
  ELSE
    delivery_fee_amount := round(
      delivery_base_fee
      + GREATEST(0, distance_amount - delivery_base_km)
        * delivery_per_km_fee,
      2
    );
  END IF;
  ...
  surge_fee_amount :=
    round(
      delivery_fee_amount * GREATEST(0, surge_multiplier - 1),
      2
    );

  tax_amount :=
    round(
      subtotal_amount * GREATEST(tax_rate, 0),
      2
    );

  seller_commission_amount :=
    round(
      subtotal_amount * GREATEST(seller_commission_rate, 0),
      2
    );

  -- Rider platform fees are NOT rider earnings.
  -- They are calculated and deducted separately when the rider accepts.
