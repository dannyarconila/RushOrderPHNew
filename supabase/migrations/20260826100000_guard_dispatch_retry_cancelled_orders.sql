-- Prevent retrying a dispatch job when its order is no longer ready.
--
-- This is a defense-in-depth guard. The cancellation trigger already marks
-- the dispatch job cancelled, but retry must independently verify the
-- authoritative order status before creating another rider offer.
...
BEGIN
  ...
  -- The order is the authoritative source of truth.
  -- Never retry or redispatch a cancelled/non-ready order.
  SELECT o.status
  INTO order_status
  ...
  IF NOT FOUND OR order_status <> 'ready' THEN
    ...
    SET
      status = CASE
        WHEN order_status = 'cancelled' THEN 'cancelled'::dispatch_job_status
...
COMMENT ON FUNCTION public.dispatch_retry(uuid)
IS 'Retries rider dispatch only while the associated order remains ready.';
